...
import { Eye, Search, Pen } from "lucide-react";
...
if(res.status===405){navigate('/admin');}
        ...
        if (res.ok) {
          ...
          setCardData({
            ...
            onLeave: data.filter(emp => emp.status === "onLeave").length,
 ...
 {/* 🔹 Search & Date Filter */}
      ...
      <div  key={currentPage}  className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-8    transition-opacity duration-500 ease-in-out opacity-100 animate-fade">
        ...
        <div className="bg-white shadow rounded-lg p-6 overflow-x-auto max-h-[500px]">
          <div className="table-auto w-full border-collapse text-sm text-gray-800">
         
            <div className="flex text-gray-700 font-semibold text-center select-none">
              <div className="flex-1 text-black font-bold bg-blue-100 rounded-l-xl px-6 py-3 border border-blue-300 border-r-0">
    ...
    ${sortId === "decName" ? "text-gray-400 cursor-not-allowed" : "text-blue-600"}`}
              ...
              <div className="flex-1 text-black font-bold bg-blue-100 px-6 py-3 border border-blue-300 border-r-0">
                 <div className="flex items-center">
                    <span>Position</span>
                    <div className="flex flex-col ml-2 leading-tight">
                      <span
    ...
    ${sortId === "decPosition" ? "text-gray-400 cursor-not-allowed" : "text-blue-600"}`}
              ...
              <div className="flex-1 text-black font-bold bg-blue-100 rounded-r-xl px-6 py-3 border border-blue-300">
               <div className="flex items-center">
            ...
            {filteredEmployees?.length > 0 ? (
              filteredEmployees.map((emp, idx) => (
                <div key={idx} className="flex items-center ">
                  <div className="flex-1 bg-blue-50 rounded-l-xl px-6 py-4 border border-blue-200 border-r-0 text-center font-medium text-gray-700">
                    {emp.name || "N/A"}
                  </div>
                  <div className="flex-1 bg-blue-50 px-6 py-4 border text-center border-blue-200 border-r-0 text-sm">
                    {emp.position || "N/A"}
                  </div>
                  <div className="flex-1 bg-blue-50 rounded-r-xl px-6 py-4 border text-center border-blue-200 text-sm">
                    {emp.department || "N/A"}
                  </div>
                </div>
              ))
