...
const SuperEmpOverview = () => {
  ...
  const loadEmployees = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setEmployees(data);
        setLoading(false);
        setCardData((prev)=>({...prev,total:result.total}));
      } else {
  ...
  return (
    <div className="p-6 w-full">
      ...
      <h2 className="text-2xl font-semibold mb-6 mt-8 text-gray-800">
        Employee Overview
      </h2>
      ...
      <div
        key={currentPage}
        className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-8  transition-opacity duration-500 ease-in-out opacity-100 animate-fade "
      >
